...
bool FimgApiCompromise(Fimg *fimg)
{
    ...
    // [src][dst][scale][global_alpha][filter_mode][blending_mode][dithering]
    if ((param.clipW * param.clipH) < comp_value[param.src_fmt][param.dst_fmt][param.isScaling][isG_alpha][param.isFilter][param.isSrcOver][fimg->isDither])
        return false;
